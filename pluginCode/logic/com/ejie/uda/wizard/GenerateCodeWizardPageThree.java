...
public class GenerateCodeWizardPageThree extends WizardPage {
	...
	public void createControl(Composite parent) {
		...
		// Grupo de la componentes de Negocio
		
//		GridLayout layoutGroup = new GridLayout();
//		layoutGroup.numColumns = 4;
...
//		ownerLayout.marginHeight = 5;
		...
		earClassesLocationButton.setLayoutData(new GridData (SWT.FILL, SWT.CENTER, true, true,1,1));
		
		
	
		// Radio de inyección de dependencias
		Label labelRadioPersistence = new Label(businessGroup, SWT.NULL);
		labelRadioPersistence.setText("Inyección de dependencias:");
		labelRadioPersistence.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, false, false,1,1));

		// Radio button para XML
		radXml = new Button(businessGroup, SWT.RADIO);
		radXml.setText("XML");
		radXml.setSelection(false);
		radXml.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                if (radXml.getSelection()){
                	setMessage("Este Wizard genera la estructura necesaria para el backend necesario");
                }
            }
        });
		radXml.setLayoutData(new GridData (SWT.CENTER, SWT.FILL, true, true,1,1));
		
		// Radio button Anotaciones
		radAnot = new Button(businessGroup, SWT.RADIO);
		radAnot.setText("Anotaciones");
		radAnot.setSelection(true);
		radAnot.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                if (radAnot.getSelection()){
                	setMessage("Este Wizard genera la estructura necesaria para el backend necesario");
                }
            }
        });
		radAnot.setLayoutData(new GridData (SWT.FILL, SWT.FILL, true, true,1,1));
		
		// Salto de línea
		hiddenLabel= new Label(container, SWT.NULL);
		hiddenLabel.setLayoutData(gd3);
		
		// Grupo de la Componentes de Presentacion
		...
		// Radio de inyección de dependencias
		Label labelRadioControl = new Label(presentationGroup, SWT.NULL);
		labelRadioControl.setText("Inyección de dependencias:");
		labelRadioControl.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, false, false,1,1));

		// Radio button para XML
		radXmlControl = new Button(presentationGroup, SWT.RADIO);
		radXmlControl.setText("XML");
		radXmlControl.setSelection(false);
		radXmlControl.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                if (radXmlControl.getSelection()){
                	setMessage("Este Wizard genera la estructura necesaria para el backend necesario");
                }
            }
        });
		radXmlControl.setLayoutData(new GridData (SWT.CENTER, SWT.FILL, true, true,1,1));
		
		
		// Radio button Anotaciones
		radAnotControl = new Button(presentationGroup, SWT.RADIO);
		radAnotControl.setText("Anotaciones");
		radAnotControl.setSelection(true);
		radAnotControl.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                if (radAnotControl.getSelection()){
                	setMessage("Este Wizard genera la estructura necesaria para el backend necesario");
                }
            }
        });
		
		radAnotControl.setLayoutData(new GridData (SWT.FILL, SWT.FILL, true, true,1,1));
	...
	/**
	 * Recupera el valor del radio de Inyección de dependencias Anotaciones del control
	 * 
	 * @return true si seleccionado, false ecc.
	 */
	public boolean getRadAnotControl() {
		return radAnotControl.getSelection();
	}
	/**
	 * Recupera el valor del radio de Inyección de dependencias XML del control
	 * 
	 * @return true si seleccionado, false ecc.
	 */
	public boolean getRadXmlControl() {
		return radXmlControl.getSelection();
	}
...
public boolean getConfirmation(){
	...
	String packageName = "";
	if (getEARClassesProject()!=null){
		packageName= "com.ejie."+ getEARClassesProject().getName().replace(Constants.EARCLASSES_NAME,"");
	}else{
		packageName="com.ejie."+ getAppName(getWarProject());
	}
...
private String getAppName(IProject project){
	...
	while (!found){
		...
		if (aux.equals("A")||aux.equals("B")||aux.equals("C")||
				...
				aux.equals("J")||aux.equals("J")||aux.equals("L")||
				aux.equals("M")||aux.equals("Ñ")||aux.equals("O")||
				aux.equals("P")||aux.equals("Q")||aux.equals("R")||
				aux.equals("S")||aux.equals("T")||aux.equals("U")||
				aux.equals("V")||aux.equals("W")||aux.equals("X")||
				aux.equals("Y")||aux.equals("Z")
				){
