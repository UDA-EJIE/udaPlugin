...
public class DataBaseWorker {
	...
	public static JDBCMetaDataConfiguration getConfiguration(ConnectionData conData){
		...
		jmdc.setProperty("hibernate.dialect",
	...
	public static ReverseEngineeringStrategy getReveng(){
		...
		ReverseEngineeringStrategy reveng = new Reveng(strategy);
	...
	public static TreeNode getSchemaTree(ConnectionData conData){
		...
		if (conData != null){
			...
			// Añade un reveng quitando las vistas (VIEW) del esquema
			
			ReverseEngineeringStrategy reveng = getReveng();
			...
			while (ic.hasNext()) {
				...
				while (properties.hasNext()) {
					...
					if (!c2h.isCollection(prop) && !c2j.isComponent(prop) && (!c2h.isOneToOne(prop)) ) {
						...
						Iterator propiedades = c2h
								.getProperties((Component) prop.getValue());
